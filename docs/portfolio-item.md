...
- [x] **P8** — Vault UI evidence — CLI form captured 2026-05-18 → `docs/portfolio-item-assets/p8-vault-cli-evidence.txt` (Vault `vault-k8s-ref-demo-0` Running v1.19.0, unsealed, KV `secret/eso-source-config` populated at version 2 incl. rotation roundtrip, kubernetes auth role `k8s-ref-demo` bound to `k8s-ref-demo/eso-reader`); UI PNG `p8-vault-ui.png` pending port-forward + browser session
